<script lang="ts">
  ...
  import { spring } from 'svelte/motion';
	...
	import { reservationData } from "../../util/store";
  ...
  function handleSave(){
    if(nameValue?.length > 0 && phoneValue?.length > 0 && count > 0 && formattedDate?.length > 0){
      ...
      reservationData.update(reservations => {
        ...
        let data = {
          id:reservations[reservations?.length-1]?.id ? reservations[reservations?.length-1]?.id + 1 : reservations[reservations?.length-1]?.id === 0? reservations[reservations?.length-1]?.id+1:0,
...
<div class="add-container">
  ...
  <div class="line-box">
    <button class="save-btn" disabled={!(nameValue?.length > 0 && phoneValue?.length > 0 && count > 0 && formattedDate?.length > 0)} type="submit" on:click={handleSave}>Save</button>
  </div>
</div>
